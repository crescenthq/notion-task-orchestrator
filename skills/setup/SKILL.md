...
(creating a Notion integration, pasting a token, choosing a pipe shape).
...
- **PR review pipe** — a planner reads the diff → a security agent scans for
...
- **Spec-to-code pipe** — a product agent writes a spec from a one-liner → an
...
then use the **add-factory** skill to create the TypeScript pipe file.

  ...
  .superRefine((retry, ctx) => {
    ...
    if (hasMax === hasLegacyMax) {
      ctx.addIssue({
        ...
        message:
          'Retry config must define exactly one of `max` or `maxRetries`',
        path: ['max'],
      })
    }
  })
...
const orchestrateStateSchema = z
  .object({
...
export function defineFactory<TFactory extends FactoryDefinition>(
  factory: TFactory,
): TFactory {
  return factory
}

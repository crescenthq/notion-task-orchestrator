...
export type TransitionEventReasonCode = z.infer<typeof transitionEventReasonSchema>;

const nonEmptyString = z.string().trim().min(1);
const isoTimestamp = nonEmptyString.refine((value) => !Number.isNaN(Date.parse(value)), {
      ...
      case "action.attempt.error":
        ...
        if (value.fromStateId !== value.toStateId) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ["toStateId"],
            message: "action attempt events must keep fromStateId and toStateId equal",
          });
        }
        break;
      case "loop.continue":
...
export function replayTransitionEvents(events: ReadonlyArray<unknown>): string | null {

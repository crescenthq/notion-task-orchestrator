...
describe("loadFactoryFromPath", () => {
  ...
  it("rejects imported runtime function references", async () => {
    ...
    await writeFile(helperPath, `export async function importedAgent() { return { status: "done" }; }\n`, "utf8");
    await writeFile(
      filePath,
    ...
    await expect(loadFactoryFromPath(filePath)).rejects.toThrow(/Imported function `importedAgent` cannot be used as runtime `agent`/);
  });
});

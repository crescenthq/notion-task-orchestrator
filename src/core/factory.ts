...
export function assertFactoryRuntimeFunctionsAreLocal(
  sourceText: string,
  filePath: string,
): void {
  ...
  for (const match of sourceText.matchAll(RUNTIME_SLOT_REGEX)) {
    ...
    if (importedIdentifiers.has(identifier)) {
      violations.add(
        `Imported function \`${identifier}\` cannot be used as runtime \`${slot}\``,
      )
    }
  }
...
export function serializeFactoryDefinition(
  definition: FactoryDefinition,
): string {

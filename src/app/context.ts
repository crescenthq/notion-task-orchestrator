...
import { resolveRuntimePaths, type RuntimePaths } from "../config/paths";
...
export async function openApp(options: OpenAppOptions = {}) {
  ...
  await appendFile(runtimePaths.runtimeLog, `${nowIso()} openApp initialized\n`, "utf8");

  return { db, client, paths: runtimePaths };
}

async function resolveAppRuntimePaths(options: OpenAppOptions): Promise<RuntimePaths> {
  if (options.projectRoot && options.projectRoot.trim().length > 0) {
    return resolveRuntimePaths(options.projectRoot);
  }

  if (process.env.NOTIONFLOW_PROJECT_ROOT && process.env.NOTIONFLOW_PROJECT_ROOT.trim().length > 0) {

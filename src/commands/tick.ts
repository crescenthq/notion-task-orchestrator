...
type TickLoopDeps = {
  runTick: (options: Omit<TickExecutionOptions, "loop" | "loopDelayMs">) => Promise<void>;
  createLogger: (options: { configPath?: string; startDir: string }) => Promise<TickRuntimeLogger>;
  random: () => number;
...
const defaultTickLoopDeps: TickLoopDeps = {
  ...
  createLogger: async (options) => {
    const { paths } = await openApp({ configPath: options.configPath, startDir: options.startDir });
    return {
      runtimeLog: async (line) => {
        await appendFile(paths.runtimeLog, `${new Date().toISOString()} ${line}\n`, "utf8");
      },
      errorLog: async (line) => {
        await appendFile(paths.errorsLog, `${new Date().toISOString()} ${line}\n`, "utf8");
      },
    };
  },
  random: () => Math.random(),
...
function parseTickExecutionOptions(args: TickArgs): TickExecutionOptions {
  ...
  const intervalMs = args.intervalMs ? Number.parseInt(String(args.intervalMs), 10) : undefined;
...
function computeBackoffMs(attempt: number, random: () => number): number {
  ...
  const jitterMs = Math.floor(Math.max(0, Math.min(1, random())) * (BACKOFF_JITTER_MAX_MS + 1));
  ...
  try {
    while (!stopRequested) {
      ...
      try {
        ...
        await logger.errorLog(`tick cycle failed index=${cycleCount} error=${message}`);

        if (!isRetryableNotionApiError(error)) {
          await logger.runtimeLog(`tick loop aborting on non-retryable error index=${cycleCount}`);
          throw error;
        }
      ...
      await logger.runtimeLog(`tick cycle sleep index=${cycleCount} delay_ms=${options.loopDelayMs}`);

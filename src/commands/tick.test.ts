...
describe("tick command loop execution", () => {
  ...
  it("runs one cycle in one-shot mode by default", async () => {
    ...
    await executeTick(
      ...
      },
      {
        ...
        random: () => 0,
        signalSource: new EventEmitter() as unknown as Pick<NodeJS.Process, "on" | "off">,
      },
    );
  ...
  it("applies exponential backoff with cap for retryable Notion API errors", async () => {
    ...
    const execution = executeTick(
      ...
      },
      {
        runTick: async () => {
          runCalls += 1;
          if (runCalls === 1) throw new Error("Notion query failed (429): limited");
          if (runCalls === 2) throw new Error("Notion query failed (503): unavailable");
          if (runCalls === 3) throw new Error("Notion query failed (502): bad gateway");
    ...
    const backoffLines = logger.runtimeLines.filter((line) => line.includes("tick cycle retryable error"));
    ...
    expect(logger.runtimeLines.some((line) => line.includes("tick loop stopped cycles=4"))).toBe(true);
  });
  ...
  it("completes the current cycle after SIGINT and exits without scheduling a new cycle", async () => {
    ...
    expect(runCalls).toBe(1);
    expect(logger.runtimeLines.some((line) => line.includes("termination requested signal=SIGINT"))).toBe(true);
    expect(logger.runtimeLines.some((line) => line.includes("tick loop stopped cycles=1"))).toBe(true);
  });
});

...
export class ProjectConfigResolutionError extends Error {
  ...
  constructor(message: string, options: { startDir: string; attemptedPath?: string }) {
    super(message);
...
export async function resolveProjectConfig(options: ResolveProjectConfigOptions = {}): Promise<ResolvedProjectConfig> {
  ...
  if (explicitConfigPath) {
    ...
    const validatedPath = await validateConfigPath(resolvedExplicitPath, startDir);
    return {
...
async function validateConfigPath(configPath: string, startDir: string): Promise<string> {
  try {
    const configStat = await stat(configPath);
    if (!configStat.isFile()) {
      throw new ProjectConfigResolutionError("Provided --config path is not a file.", {
        startDir,
    ...
    throw new ProjectConfigResolutionError("Provided --config path does not exist.", {
      startDir,
      attemptedPath: configPath,
    });
  }

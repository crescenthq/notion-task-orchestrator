...
describe("local project init", () => {
  ...
  it("scaffolds config, factories, runtime dir, and idempotent gitignore entry", async () => {
    ...
    await expect(stat(path.join(fixture.projectDir, "notionflow.config.ts"))).resolves.toBeTruthy();
    await expect(stat(path.join(fixture.projectDir, "factories"))).resolves.toBeTruthy();
    await expect(stat(path.join(fixture.projectDir, ".notionflow"))).resolves.toBeTruthy();

    const gitignore = await readFile(path.join(fixture.projectDir, ".gitignore"), "utf8");

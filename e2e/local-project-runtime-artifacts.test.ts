...
describe("local project runtime artifacts", () => {
  ...
  it("writes db and runtime logs under project-local .notionflow with no global writes", async () => {
    ...
    await expect(stat(path.join(runtimeDir, "notionflow.db"))).resolves.toBeTruthy();
    await expect(stat(path.join(runtimeDir, "runtime.log"))).resolves.toBeTruthy();
    await expect(stat(path.join(runtimeDir, "errors.log"))).resolves.toBeTruthy();
